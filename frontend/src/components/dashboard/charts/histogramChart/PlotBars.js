...
const PlotBars = ({ chartData, selection, setSelection }) => {
    ...
    useEffect(() => {
        ...
        const xScale = d3.scaleLinear().domain([axes.minX, axes.maxX]).range([0, width]);
        const yScale = d3.scaleLinear().domain([axes.maxY, 0]).range([0, height]);
        ...
        chartData.forEach((v, i) => {
            ...
            context.fillRect(xScale(v.x0), yScale(v.length), rectWidth, height-yScale(v.length));
            context.strokeRect(xScale(v.x0), yScale(v.length), rectWidth,  height-yScale(v.length));
        });
